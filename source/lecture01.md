...
var:
  ...
  header-date: "2026年04月06日 (月) 3時限〔旧5-6時限〕"
---

# 第01回 2I-プログラミング1

<!-- 以下のコンテンツはダミーです。別科目のコンテンツです -->

---
var:
  header-title: "2025-2I プログラミング1 第23回 講義資料"
  header-date: "2025年12月15日（月）5・6時限"
---
...
現在までの **_Pythonプログラミングに関する学び (9カ月弱) の集大成_** として、**何らかのPythonアプリ**(自由作品) を作成し、そのプログラムを **GitHub で公開**するとともに、[課題07](lecture18.html#課題07-ポートフォリオ制作)として作成した**「ポートフォリオ」に掲載**してください。

<div class="note type-tips">
**参考**

作成に際しては先輩方のポートフォリオの該当項目 (制作期間が 2024年12月～2025年1月、もしくは 2023年12月～2024年1月 になっているもの) を参考にしてください。

- [2024-2I](https://omunet-my.sharepoint.com/:t:/g/personal/z21707r_omu_ac_jp/EfHzQGGsCxtJlZhTipZNje8B2dehojbvIL8jh0Ors6ej-A?e=qA1iMy) (学内のみ OneDrive)
- [2024-3I](https://omunet-my.sharepoint.com/:t:/g/personal/z21707r_omu_ac_jp/EQs46EyT1wZIs1ZeWU7l_MABf3rdDQkPqQgkQBTUk5ZqBw?e=uCvAgr) (学内のみ OneDrive)

</div>

- **_提出物・提出先_** : 作品の **GitHubリポジトリのURL** (≠ポートフォリオURL) を Teams に提出してください。
  - なお、ポートフォリオに作品が掲載されているかは[2025-PG1-課題07の作品共有](https://omunet-my.sharepoint.com/:t:/g/personal/z21707r_omu_ac_jp/EcJwJ2lr6IxFozn_2lk7m6MB6PySj6NiE7ZfZXXHDbLnXQ?e=gVwSVv)から確認します。もし、URLが変更されている場合は Teamsのチャットで連絡をください。
- **_提出期限_** : **2026年01月18日 (日) 23時**
  - 上記の期日以降に採点します。課題の採点後は、修正版の再提出・追提出があっても原則として再評価はしません (＝採点がフィードバックされるまでは <u>リンク先の内容</u> を更新して問題ありません)。
  - 提出遅れは、大幅な減点となります。また、上記期限から168時間経過後しても Teams にリンクが未提出の場合は「0点」で確定します。以降の提出は受け付けません。
  - 取組みの目安となる時間: **14時間以上** (=1日1時間 x 冬休み日数)。
  - 本科目には中間試験や期末試験などの筆記試験がありません。それらの試験勉強時間の代替として本課題に取り組んでください。
- **_評価_** : GitHubリポジトリの **README.md** の内容を「5点満点」、プログラム (プロジェクト構成) を「5点満点」で、**合計10点満点** で評価します。**7.6点** を標準とします。

- 提出された課題は**知能情報コースの学生と教員に公開**します。

### 注意1

**README.md** には、必ず次の内容を含めてください。これらに漏れがあると大きな減点となります。細かな書式や形式については自由です。

- 制作時期として「2025年12月～2026年1月」のような情報を明記してください。日付のフォーマットは自由です (例えば「2025/12」や「December 2025」などでもOKです)。この制作時期の情報から、それが本科目の「課題08」であることを識別します。 
- 作品の「**タイトル**」と「**説明文章**」を書いてください。説明文章は、最低でも **400文字以上** を記載してください。なお、説明のなかに「課題08」のような言葉は**不要**です。
- **5枚以上のスクリーンショット**を載せてください。スクリーンショットの代わりに**動画やGIFアニメーションなどでも可**です。ただし、GitHubには 25MB を超える動画はアップできないので注意してください (必要に応じてYouTubeなどにアップして、それをページに埋め込みしてください)。
- README.md を構成するマークダウンが <u>適切にレンダリングをされていること</u> を必ず GiyHub 上で確認してください。
  - 「**\## 制作時期**」や「**\*\*使用ライブラリ\*\***」のように、<u>マークダウン書式が適切にレンダリングされていない箇所があれば、見直しせずに提出しているとして大幅減点</u>します。

README.md については、[先輩が作成した課題](https://omunet-my.sharepoint.com/:t:/g/personal/z21707r_omu_ac_jp/EbU8myRl3IJJny0f_QNRyHIBanMY_sgtuBABoS0nf1rszg?e=7J9XuH)などを参考にしてください。

- GitHub のファイル一覧から README.md を選択して「Code」を参照すれば、どのように記述しているかを確認できます。

### 注意2

Python プロジェクト は、次の事項に注意して構成してください。

- フォルダやファイルに日本語を使用しないでくだい。
- ここまでに学んだ「ファイルの分割」や「クラス」を活用してください。
- プロジェクトのルートに `README.md` と `requirements.txt` を配置してください。
  - `requirements.txt` の作成方法は、[第22回講義](lecture22.html#環境構築)や生成AIを参考にしてください。
- GitHub にプッシュするとき、`.venv` や `__pycache__` などが含まれないように、適切に `.gitignore` を設定してください。`.gitignore` については[第16回講義](lecture16.html#アリス役のステップ-2)や生成AIを参考にしてください。

**(プロンプト例)**

> 学校で「Pythonアプリを開発し、それを GitHub にプッシュし、リポジトリのURLを提出せよ」という課題がでました。そのなかに「プロジェクトのルートに `README.md` を配置してください。」という指示がありました。この指示の意味や意図を、Python学習9ヶ月目の私に詳しく解説してください。

**(プロンプト例)**

> 学校で「Pythonアプリを開発し、それを GitHub にプッシュし、リポジトリのURLを提出せよ」という課題がでました。そのなかに「プロジェクトのルートに `requirements.txt` を配置してください。」という指示がありました。この指示の意味や意図を、Python学習9ヶ月目の私に詳しく解説してください。

**(プロンプト例)**

> 学校で「Pythonアプリを開発し、それを GitHub にプッシュし、リポジトリのURLを提出せよ」という課題がでました。そのなかに「GitHub にプッシュするとき `.venv` や `__pycache__` などが含まれないように、適切に `.gitignore` を設定してください。」という指示がありました。この指示の意味や意図を、Python学習9ヶ月目の私に詳しく解説してください。

**(プロンプト例)**

> GitHub リポジトリのトップにブラウザでアクセスしたときに表示される `README.md` のなかに、画像を掲載したいです。どのようにすればよいですか。

### Q&A

その他、不明点があれば **TeamsChat** で相談・確認をしてください。

- **Q**: フリー素材を使ってアプリを作成しようと考えています。しかし、プロジェクトに使用した「フリー素材」を GitHubリポジトリにアップするとフリー素材の「利用規約」で禁止されている「再配布」に該当してしまうかもしれません。どうしたらよいですか。
  - **A**: 画像や音楽などのデータについては、必ずしも GitHub にアップする必要はありません。[第16回講義](lecture16.html#アリス役のステップ-2)でも紹介しているように `.gitignore` で、それらのデータをGit管理から除外してしください。
- **Q**: Jupyter / GoogleColab のノートブック環境で動作するPythonプログラムでもよいですか。
  - **A**: 問題ありません。
- **Q**: Python以外の言語で開発したアプリでもよいですか。
  - **A**: 今回は「**不可**」です。
- **Q**: 前回講義や、今回/次回講義のなかで作成したゲームアプリを改良した作品でもよいですか。
  - **A**: 問題ありません。
...
- 初回実行時は起動に失敗することがあります。その場合、再実行してください。
...
**演習2**: `05_game2_01.py` では「**48px**」を基準にゲーム画面が構成、描画されている。これを「24px」あるいは「72px」を基準にゲームが画面が構成、描画されるように変更せよ (つまり、**ゲーム画面の表示倍率変更** をせよ)。例えば `scale_factor` という定数を設け、それが `1` のときは「24px」を基準に、また `2` のときは「48px」を基準に、また `3` のときは「72px」を基準にゲームが画面が構成、描画されるように変更せよ。実装例は[こちら](https://github.com/TakeshiWada1980/Programming1-2024/blob/main/docs/code/23/05_game2_01-a.py)。
...
- ChatGPT:オブジェクト指向プログラミングについて教えてください

**(プロンプト例)**

> 私は情報系学科の高専2年生です。約8カ月、授業のなかで「Pythonプログラミング」について学んできました。しかし、「オブジェクト指向プログラミング」については、まだ学んでいません。オブジェクト指向プログラミングというものについて教えてください。
...
> Python の クラスにおける「プロパティ」と「属性」の違いについて教えてください。
...
- 実装例 (解答例) は[こちら](https://github.com/TakeshiWada1980/Programming1-2024/blob/main/docs/code/23/MyVector2.py)。
